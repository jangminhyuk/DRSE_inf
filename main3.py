...
are returned. These are averaged over simulation runs and experiments, and the final
results are saved separately for each robust parameter value.
    
Usage example:
    python main3.py --dist normal --noise_dist normal --num_sim 500 --horizon 20 --num_exp 10
"""
...
def enforce_positive_definiteness(Sigma, epsilon=1e-6):
    ...
    if min_eig < epsilon:
        ...
        Sigma += adjustment * np.eye(Sigma.shape[0])
    return Sigma
...
def compute_lqr_cost(result, Q_lqr, R_lqr, K_lqr):
    x = result['state_traj']         # true state trajectory, shape (T+1, nx, 1)
    x_est = result['est_state_traj']   # estimated state trajectory, shape (T+1, nx, 1)
    T = x.shape[0] - 1
    cost = 0.0
    for t in range(T):
        u = -K_lqr @ x_est[t]
...
def main(dist, noise_dist, num_sim, T, num_exp):
    seed_base = 2024
    robust_vals = [0.01, 0.05, 0.1, 0.2, 0.5]
    all_results = {}
