...
Usage example:
    python main7.py --dist normal --noise_dist normal --num_sim 1 --num_exp 5 --time 10 --trajectory curvy
"""
...
def run_experiment(exp_idx, dist, noise_dist, num_sim, seed_base, robust_val, T_total, desired_traj):
...
def main(dist, noise_dist, num_sim, num_exp, T_total, trajectory):
    ...
    for robust_val in robust_vals:
        ...
        experiments = Parallel(n_jobs=-1)(
            delayed(run_experiment)(exp_idx, dist, noise_dist, num_sim, seed_base, robust_val, T_total, desired_traj)
    ...
    save_data(os.path.join(results_path, f'raw_experiments_{dist}_{noise_dist}.pkl'), raw_experiments_data)
    print("LQR with state estimation experiments completed for all robust parameters.")
    
    # --- Plot Representative 2D Trajectories ---
    # For non-robust filters ('finite' and 'inf'), use the first robust candidate.
    # rep_state_trajs = {}
    # for filt in filters:
    ...
    #     rep_state_trajs[filt] = all_results[robust_val_for_filter]['state'][filt]
