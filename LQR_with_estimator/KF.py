...
with the control input computed by an LQR controller:
    uₜ = -K_lqr x_est[t].
The filter uses the known mean values (for x0, process noise, and measurement noise)
and the EM–estimated covariances for its prediction and update.
