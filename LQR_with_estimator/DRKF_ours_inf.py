...
in a closed-loop LQR experiment. The system evolves as:
    xₜ₊₁ = A xₜ + B uₜ + wₜ,    yₜ = C xₜ + vₜ,
with wₜ and vₜ sampled from the true distributions. A worst–case measurement noise covariance
is computed via an SDP. In this revised version, a control input is incorporated via an LQR
controller, where u[t] = -K_lqr * x_est[t] (with B passed to the filter and K_lqr assigned externally).
"""
